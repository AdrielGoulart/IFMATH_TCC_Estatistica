...
@Component({
  ...
  templateUrl: './cf-relacoes-metricas-rst.component.html',
  styleUrls: ['./cf-relacoes-metricas-rst.component.scss']
})
export class CfRelacoesMetricasRstComponent implements AfterViewInit {
  ...
  distancePA = 0; distancePB = 0; distancePC = 0;
  private result1 = 0; private result2 = 0;
  ...
  ngDoCheck(){
    this.result1 = Math.round(this.distancePB**2);
  ...
  */
  ...
  mouseMove(event: MouseEvent) {
    ...
    if (this.dragging && this.currentIndex !== null) {
      ...
      this.insersects.forEach(el => {
        if (this.currentIndex.length < 2 && (el.object.name == "ponto-a" || el.object.name == "ponto-b"
          || el.object.name == "ponto-c" || el.object.name == "ponto-p")) {
  ...
  reDrawPointsToCircle() {
    ...
    if (this.dragged === "P") {
      ...
      let dist = Math.sqrt(((circlePointA.position.x) ** 2 + (circlePointA.position.y) ** 2));
      let distancesRatio = (this.radiusExt - 0.5) / dist;
      ...
      let x = (distancesRatio * circlePointA.position.x), y = (distancesRatio * circlePointA.position.y);
      ...
      for (let i = 0; i <= 360; i++) {
        ...
        let cross = this.geoScene.lineCross(pos.x, pos.y, vertexPoint.position.x, vertexPoint.position.y, circlePointA.position.x, circlePointA.position.y, 2);
        if (cross) {
      ...
      if (!(posC.x <= vertexPoint.position.x + 1.5 && posC.x >= vertexPoint.position.x - 1.5)) circlePointC.position.copy(posC);

      // https://stackoverflow.com/a/15846131 -> achar os pontos tangentes de uma circunferencia dados o centro da circ. e um ponto P
      let dx = -vertexPoint.position.x;
      let dy = -vertexPoint.position.y;
      let dd = Math.sqrt(dx * dx + dy * dy);
      let a = Math.asin((this.radiusExt - 0.5) / dd);
      let b = Math.atan2(dy, dx);

      circlePointB.position.copy(new THREE.Vector3((this.radiusExt - 0.5) * -Math.sin(b + a), (this.radiusExt - 0.5) * Math.cos(b + a), 0));
      ...
      let distance = Math.sqrt((
        (circlePointB.position.x) ** 2 + (circlePointB.position.y) ** 2 //distancia entre o ponto zero e V
      ));
      let distancesRatio = (this.radiusExt - 0.5) / distance; //razao entre a distancia total e o raio da circunferencia (no caso, o raio seria a 'dt' na formula do link acima)

      let position = new THREE.Vector3((distancesRatio * circlePointB.position.x), (distancesRatio * circlePointB.position.y), 0);
    ...
    let theta1 = (1 / 360) * Math.PI;
    let pos = new THREE.Vector3(Math.cos(theta1), Math.sin(theta1), 0);

    let theta = 90 * 0.0175;
    //https://math.stackexchange.com/a/814981 -> formula para rotacionar um ponto em um circulo dado um angulo
    let xrot1 = Math.cos(theta) * (pos.x - circlePointB.position.x) - Math.sin(theta) * (pos.y - circlePointB.position.y) + circlePointB.position.x,
      yrot1 = Math.sin(theta) * (pos.x - circlePointB.position.x) + Math.cos(theta) * (pos.y - circlePointB.position.y) + circlePointB.position.y,
      xrot359 = Math.cos(-theta) * (pos.x - circlePointB.position.x) - Math.sin(-theta) * (pos.y - circlePointB.position.y) + circlePointB.position.x,
      yrot359 = Math.sin(-theta) * (pos.x - circlePointB.position.x) + Math.cos(-theta) * (pos.y - circlePointB.position.y) + circlePointB.position.y;

    let distance1 = Math.sqrt(((xrot1 - vertexPoint.position.x) ** 2 + (yrot1 - vertexPoint.position.y) ** 2));
    let distance359 = Math.sqrt(((xrot359 - vertexPoint.position.x) ** 2 + (yrot359 - vertexPoint.position.y) ** 2));

    if (distance1 >= distance359) circlePointD.position.copy(new THREE.Vector3(xrot1, yrot1, 0));
    else circlePointD.position.copy(new THREE.Vector3(xrot359, yrot359, 0));
    ...
    let a2 = circlePointB.position.y - circlePointD.position.y;
    let b2 = circlePointD.position.x - circlePointB.position.x;
    ...
    let distanceVB = Math.sqrt((
      (circlePointB.position.x - vertexPoint.position.x) ** 2 + (circlePointB.position.y - vertexPoint.position.y) ** 2
    ));
    let distancesRatioVB = (distanceVB + this.radiusExt) / distanceVB;

    let xCA = (1 - distancesRatioCA) * circlePointC.position.x + distancesRatioCA * circlePointA.position.x,
      ...
      xVB = (1 - distancesRatioVB) * vertexPoint.position.x + distancesRatioVB * circlePointB.position.x,
      yVB = (1 - distancesRatioVB) * vertexPoint.position.y + distancesRatioVB * circlePointB.position.y;

    let geometryLineVB = new THREE.Geometry();
    geometryLineVB.vertices.push(circlePointB.position);
    geometryLineVB.vertices.push(vertexPoint.position);

    let geometryDashedLineVB = new THREE.Geometry();
    geometryDashedLineVB.vertices.push(circlePointB.position);
    geometryDashedLineVB.vertices.push(new THREE.Vector3(xVB, yVB, 0));

    let dashedVB;
    dashedVB = new THREE.LineDashedMaterial({ color: 0xff0000, linewidth: 2, dashSize: 2, gapSize: 2 });
    ...
    let lineVB = new THREE.Line(geometryLineVB, new THREE.LineBasicMaterial({ color: 0xff0000, linewidth: 2 }));
    let lineDashedVB = new THREE.Line(geometryDashedLineVB, dashedVB); lineDashedVB.computeLineDistances();

    let bdGroup = new THREE.Group(); bdGroup.add(lineVB, lineDashedVB);
    ...
    let AB = alpha - beta, AC = alpha - charlie, BA = beta - alpha, BC = beta - charlie;
    if (AB < 0) AB += 360; if (AC < 0) AC += 360; if (BA < 0) BA += 360; if (BC < 0) BC += 360;
  ...
  distanceBetweenPoints() {
    ...
    let theta = 90 * 0.0175;
    //https://math.stackexchange.com/a/814981 -> formula para rotacionar um ponto em um circulo dado um angulo
    let xrot1 = Math.cos(theta) * (pos.x - xB) - Math.sin(theta) * (pos.y - yB) + xB,
      yrot1 = Math.sin(theta) * (pos.x - xB) + Math.cos(theta) * (pos.y - yB) + yB,
      xrot359 = Math.cos(-theta) * (pos.x - xB) - Math.sin(-theta) * (pos.y - yB) + xB,
      yrot359 = Math.sin(-theta) * (pos.x - xB) + Math.cos(-theta) * (pos.y - yB) + yB;

    let distance1 = Math.sqrt(((xrot1 - vertexPoint.position.x) ** 2 + (yrot1 - vertexPoint.position.y) ** 2));
    let distance359 = Math.sqrt(((xrot359 - vertexPoint.position.x) ** 2 + (yrot359 - vertexPoint.position.y) ** 2));
    let xD, yD;

    if (distance1 > distance359) xD = xrot1, yD = yrot1;
    else xD = xrot359, yD = yrot359;
    ...
    this.distancePB = Math.sqrt((
      this.distancePA*this.distancePC
    ));
  ...
  reDrawAngle() {
    ...
    let rotationDA = beta * 0.00875 + alpha * 0.00875 + this.angle * 0.0175 + (90 - this.angle) * 0.0175;
    let rotationCordaA = alpha * 0.0175 + angleModA * 0.0175;
    if (distanceA < distanceC) rotationCordaA = charlie * 0.0175 + angleModA * 0.0175
    let rotationCordaB = beta * 0.0175 + angleModB * 0.0175
    ...
    if (rotationDA.toString() !== 'NaN') {
      ...
      this.geoScene.scene.children[19] = new THREE.Line(geometryAngleCorda, materialAngle);
    ...
    this.geoScene.scene.children[17].position.copy(new THREE.Vector3(vertexPoint.position.x, vertexPoint.position.y - 4, 0));
  ...
  ngAfterViewInit() {
    ...
    this.geoScene.scene.children[6].position.copy(new THREE.Vector3(-3, -40, 0));
    ...
    geometryBD.vertices.push(new THREE.Vector3(-3, -40, 0));
    ...
    // P
    this.geoScene.scene.children[17] = new SpriteText('P', 4, '0x000000');
    this.geoScene.scene.children[17].position.copy(new THREE.Vector3(18, -4, 0));
    this.geoScene.scene.children[17].name = "label-p";
    // Alpha
    this.geoScene.scene.children[18] = new SpriteText('α', 4, '#FF0000');
    this.geoScene.scene.children[18].position.copy(new THREE.Vector3(10, 0, 0));

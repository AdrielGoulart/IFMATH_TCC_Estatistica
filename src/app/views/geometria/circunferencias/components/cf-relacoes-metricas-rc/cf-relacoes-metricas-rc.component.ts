...
@Component({
  ...
  templateUrl: './cf-relacoes-metricas-rc.component.html',
  styleUrls: ['./cf-relacoes-metricas-rc.component.scss']
})
export class CfRelacoesMetricasRcComponent implements AfterViewInit {
  ...
  */
  ...
  reDrawAngle() {
    ...
    let circlePointD = this.geoScene.scene.getObjectByName("ponto-d");
    let vertexPoint = this.geoScene.scene.getObjectByName("ponto-p");
    let angleC, angleCorda, angleCordaCopy, angle, angleCopy;
    ...
    if (!this.geoScene.fromInput) {
      ...
      let C = new THREE.Vector2(circlePointA.position.x - vertexPoint.position.x, circlePointA.position.y - vertexPoint.position.y);
      let D = new THREE.Vector2(circlePointA.position.x - circlePointD.position.x, circlePointA.position.y - circlePointD.position.y);

      let DC = D.x * C.x + D.y * C.y;
      let DMod = Math.sqrt(D.x ** 2 + D.y ** 2);
    ...
    for (let i = 0; i <= segmentCount; i++) {
      ...
      if (i == 30 || i == 61) {
        ...
        angleCopy = new THREE.Line(geometryLine, new THREE.LineBasicMaterial({ color: 0x000000, linewidth: 2 }));
      }
    }
    ...
    let AB = alpha - beta, AD = alpha - delta; if (AB < 0) AB += 360; if (AD < 0) AD += 360;
    ...
    let rotationCordaA = alpha * 0.0175 + (270 - angleC) * 0.0175;
    let rotationCordaB = beta * 0.0175 + (angleC - 90) * 0.0175;
    if (AD < AB) {
    ...
    if (rotationDA.toString() !== 'NaN') {
      ...
      this.geoScene.scene.children[13] = new THREE.Line(geometryAngle, materialAngle);
      ...
      this.geoScene.scene.children[22] = new THREE.Line(geometryAngleCorda, materialAngle);
      ...
      this.geoScene.scene.children[23] = new THREE.Line(geometryAngleCorda, materialAngle);
  ...
  ngAfterViewInit() {
    ...
    geometryAC.vertices.push(new THREE.Vector3(-24, 22.75, 0));
    geometryAC.vertices.push(new THREE.Vector3(31, -7, 0));
    const geometryBD = new THREE.Geometry();
    geometryBD.vertices.push(new THREE.Vector3(31, 7, 0));
    geometryBD.vertices.push(new THREE.Vector3(-24, -22.75, 0));
    const geometryAD = new THREE.Geometry();
    geometryAD.vertices.push(new THREE.Vector3(-21, 21, 0));
    geometryAD.vertices.push(new THREE.Vector3(-21, -21, 0));
    const geometryBC = new THREE.Geometry();
    geometryBC.vertices.push(new THREE.Vector3(29, 6, 0));
    geometryBC.vertices.push(new THREE.Vector3(29, -6, 0));
    let dashed;
    dashed = new THREE.LineDashedMaterial({ color: 0x0000ff, linewidth: 2, dashSize: 2, gapSize: 2 });
    let lineAD = new THREE.Line(geometryAD, dashed); lineAD.computeLineDistances();
    ...
    // Ângulo Alpha
    this.geoScene.scene.children[12] = new THREE.Line(geometryAngleAlpha, new THREE.LineBasicMaterial({ color: 0x0000FF, linewidth: 2 }));
    ...
    this.geoScene.scene.children[13] = new THREE.Line(geometryAngleAlpha, new THREE.LineBasicMaterial({ color: 0x0000FF, linewidth: 2 }));
    this.geoScene.scene.children[13].position.copy(new THREE.Vector3(18, 0, 0));
    ...
    this.geoScene.scene.children[18].position.copy(new THREE.Vector3(-25, -25, 0));
    ...
    // Alpha Espelhado
    this.geoScene.scene.children[21] = new SpriteText('α', 4, '#FF0000');
    this.geoScene.scene.children[21].position.copy(new THREE.Vector3(26, 0, 0));
